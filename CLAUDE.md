...
(Ali's user-level CLAUDE.md — profile and global rules — still applies and is not repeated here.)
...
- `src/state/store.ts` (Zustand) — the serialisable core: tabs, settings, recent, pageSetup,
  ...
  `session.json` round-trips each tab's id / `dirty` / `encoding` / `hadBom` and reactivates
  the saved active tab, so restored unsaved edits still prompt on close (don't regress);
  each config file is parsed independently so one corrupt file can't block startup.
...
└── .claude\  skills-lock.json  ← Claude Code's own files — do not move/edit
```
